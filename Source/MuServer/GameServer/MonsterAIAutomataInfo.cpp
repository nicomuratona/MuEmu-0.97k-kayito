...
CMonsterAIState* CMonsterAIAutomataInfo::RunAutomataInfo(int aIndex)
{
	...
	if (OBJECT_RANGE(lpObj->TargetNumber) != false && CMonsterAIUtil::FindMonViewportObj(aIndex, lpObj->TargetNumber) == 0)
	{
	...
	for (int n = 0; n < this->m_AIStateTransCount[lpObj->CurrentAIState]; n++)
	{
		...
		switch (lpMonsterAIState->m_TransitionType)
		{
			...
			{
				if (OBJECT_RANGE(CMonsterAIUtil::FindMonsterToHeal(lpObj, lpMonsterAIState->m_TransitionValue, 6)) != false)
				{
					if ((GetLargeRand() % 100) < lpMonsterAIState->m_TransitionRate)
					{
						lpObj->LastAutomataDelay = lpMonsterAIState->m_DelayTime;
