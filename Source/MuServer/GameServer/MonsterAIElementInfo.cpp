...
void CMonsterAIElementInfo::ApplyElementGroupMove(int aIndex, int bIndex, CMonsterAIState* lpMonsterAIState)
{
	...
	if (OBJECT_RANGE(LeaderIndex) != false && gObj[LeaderIndex].Live != 0 && gObjCalcDistance(lpObj, &gObj[LeaderIndex]) > 6)
	{
...
void CMonsterAIElementInfo::ApplyElementAttack(int aIndex, int bIndex, CMonsterAIState* lpMonsterAIState)
{
	...
	{
		if (OBJECT_RANGE(lpObj->TargetNumber) == false || gObj[lpObj->TargetNumber].Connected != OBJECT_ONLINE || gObj[lpObj->TargetNumber].CloseCount > 0 || gObj[lpObj->TargetNumber].Live == 0 || gObj[lpObj->TargetNumber].Teleport != 0)
		{
...
void CMonsterAIElementInfo::ApplyElementAttackArea(int aIndex, int bIndex, CMonsterAIState* lpMonsterAIState)
{
	...
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		{
			gAttack.Attack(lpObj, &gObj[n], 0, 0, 0, 0);
		}
	}
}
